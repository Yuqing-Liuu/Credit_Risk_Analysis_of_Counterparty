...
4. Constant Recovery Rate – a fixed recovery rate is assumed for simplicity.  
...
The first step in simulation is to specify the stochastic behavior of the underlying market variables that determine the value of the derivative portfolio. In this research, a single market factor—such as a short-term interest rate or swap rate—is modeled as a Geometric Brownian Motion (GBM):

dS<sub>t</sub> = μS<sub>t</sub>dt + σS<sub>t</sub>dW<sub>t</sub>,

where *S<sub>t</sub>* represents the market variable at time *t*, *μ* is the drift (expected return), *σ* is the volatility, and *W<sub>t</sub>* is a Wiener process representing random shocks. The GBM process is discretized for simulation as:

S<sub>t+Δt</sub> = S<sub>t</sub> × exp[(μ − 0.5σ²)Δt + σ√Δt × Z],
...
EE(t) = (1/N) × Σ<sub>i=1</sub><sup>N</sup> E<sub>i</sub>(t).
...
EPE = (1/T) × Σ<sub>t=1</sub><sup>T</sup> EE(t),

which represents the average expected exposure over the life of the portfolio. Finally, the Effective Expected Positive Exposure (EEPE)—used for regulatory purposes—is obtained by taking the maximum of the weighted average EE values over time, thereby incorporating maturity effects:
...
- Volatility (*σ*): 20% (typical for interest rate movements)  
- Risk-free rate (*r*): 3% per annum  
- Recovery rate (*R*): 40% (consistent with industry practice)  
- Initial market value (*S₀*): normalized to 100  

These parameters provide a realistic yet computationally manageable setup. Increasing *N* improves convergence but at higher computational cost, while decreasing the time step Δt increases temporal resolution.
...
E<sub>net</sub>(t) = max(Σ<sub>j=1</sub><sup>n</sup> V<sub>j</sub>(t), 0),

where *V<sub>j</sub>(t)* is the mark-to-market value of trade *j* at time *t*, and *n* denotes the number of trades in the netting set. The summation of all *V<sub>j</sub>(t)* reflects the total portfolio value, and only the positive part is taken since negative exposure (where the bank owes the counterparty) does not contribute to credit risk from the bank’s perspective.
...
E<sub>path,net</sub>(t) = max(Σ<sub>j</sub> V<sub>j,path</sub>(t), 0).
...
E<sub>coll</sub>(t) = max(E(t) − C(t), 0),
...
E<sub>total</sub>(t) = max(Σ<sub>j=1</sub><sup>n</sup> V<sub>j</sub>(t) − C(t), 0).
...
1. Uncollateralized exposure – baseline case with no netting or collateral.  
2. Netting-adjusted exposure – captures diversification effects within portfolios.  
3. Collateralized exposure – incorporates both netting and CSA terms.  
...
| Margin Frequency | Δt | 1 day | Collateral exchanged daily |
...
However, these mechanisms are not without cost. Frequent collateral exchanges require liquidity management and impose operational burdens. Moreover, collateral does not eliminate credit risk entirely—it merely shifts it to the collateral asset’s custodian or to potential valuation mismatches during market stress. These residual risks, often referred to as “collateral liquidity risk” or “gap risk,” highlight the importance of modeling collateral processes realistically in exposure simulations (Brigo & Morini, 2010).
...
The product EE(t<sub>i</sub>) × ΔPD(t<sub>i</sub>) represents the expected loss in period i, and the summation over all periods yields the total CVA.
...
3. Obtain the counterparty’s default probability curve and compute incremental default probabilities ΔPD(t).  
4. Multiply DF(t), EE(t), and ΔPD(t) term by term, then sum over all time intervals and scale by (1 − R).
...
| Hazard rate | λ | 1.5% | Annualized default intensity from CDS spread |
| Volatility | σ | 20% | Market volatility of underlying factor |
...
The default probability term structure is generated using the hazard rate λ, assuming an exponential survival function:

PD(t) = 1 − exp(−λt).

This simple functional form ensures tractability and allows clear interpretation of credit quality. For instance, a counterparty with λ = 1.5% has an annual default probability of approximately 1.5%, corresponding to a credit rating near BBB.
...
This chapter has presented the methodological framework developed to estimate Credit Valuation Adjustment (CVA) using a Monte Carlo simulation approach. The methodology integrates theoretical concepts from counterparty credit risk modeling with practical implementation procedures inspired by the Basel Internal Model Method (IMM). It aims to capture the dynamic evolution of potential future exposures in over-the-counter (OTC) derivatives and to assess how contractual mechanisms such as netting and collateralization affect counterparty risk.

The chapter began by outlining the research design and motivation for adopting a quantitative simulation framework. The model was then constructed on solid theoretical foundations, linking market risk, credit risk, and discounting into a unified CVA formulation. The Monte Carlo simulation provided the computational means to estimate expected exposure (EE), expected positive exposure (EPE), and effective expected positive exposure (EEPE) across thousands of potential market scenarios. These exposure metrics were then used to calculate CVA as the discounted expected loss due to counterparty default.

Subsequent sections incorporated netting and collateral adjustments, demonstrating how legally enforceable agreements under ISDA frameworks significantly reduce credit exposure. The CVA computation further illustrated how credit quality, market volatility, and collateralization jointly determine the magnitude of expected credit losses. Sensitivity analysis confirmed that CVA is most responsive to default intensity and collateral effectiveness, consistent with both theoretical expectations and regulatory observations.

The model validation process established that the simulation framework is internally consistent, numerically stable, and theoretically sound within its stated assumptions. Nonetheless, certain limitations were identified, including simplified credit and market dynamics, the absence of wrong-way risk, and the reliance on hypothetical calibration parameters. These limitations provide opportunities for future refinement through the incorporation of empirical data and more sophisticated stochastic models.

Overall, the methodology developed in this chapter provides a rigorous and transparent foundation for analyzing counterparty credit exposure and CVA under varying market and contractual conditions. The next chapter applies this framework to numerical experiments and scenario analyses, presenting quantitative results that illustrate the model’s behavior and practical implications for risk management and regulatory compliance.
