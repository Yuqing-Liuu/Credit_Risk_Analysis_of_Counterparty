...
# CHAPTER III. REGULATORY FRAMEWORK FOR CVA CAPITAL CHARGE

## 3.1 Rationale for the CVA Capital Charge

The Credit Valuation Adjustment (CVA) capital charge emerged as a response to the weaknesses in pre-crisis regulatory frameworks that underestimated counterparty credit risk (CCR). Before 2008, capital regulation treated CCR primarily as the probability that a counterparty would default on contractual payments. In practice, however, the 2007–2009 global financial crisis revealed that significant losses could occur even without defaults. When counterparties’ credit spreads widened, the market value of derivative portfolios declined, producing large mark-to-market losses that were not covered by regulatory capital (Gregory, 2015; Brigo & Morini, 2010).  

Lehman Brothers’ default and AIG’s near-collapse demonstrated this gap vividly. In AIG’s case, widening credit spreads triggered collateral calls exceeding USD 30 billion, forcing emergency state intervention (Stulz, 2009). Similar valuation losses at European banks such as Dexia and Fortis also arose not from defaults, but from deterioration in counterparties’ perceived credit quality (Jorion, 2010). These events made clear that counterparty risk comprises two components: default-driven losses and market-driven credit-spread volatility. Yet the Basel II framework capitalized only the former.

The Basel Committee on Banking Supervision (BCBS) therefore introduced the CVA capital charge in Basel III to ensure that banks hold adequate capital against the mark-to-market losses arising from credit deterioration before default (BCBS, 2011). Conceptually, CVA bridges credit and market risk by linking the valuation of derivatives to the evolving creditworthiness of their counterparties. The charge aligns prudential regulation with fair-value accounting and internal pricing models that already quantify CVA and DVA (Duffie & Singleton, 2012; Green, Kenyon, & Dennis, 2014).

Beyond capital adequacy, the CVA charge also serves a macro-prudential function. Credit-spread volatility typically coincides with rising margin calls, falling collateral values, and liquidity stress (Duffie, 2010). Without a capital buffer, such feedback loops amplify market instability. By pre-positioning capital against potential spread shocks, Basel III aims to reduce procyclicality and prevent mark-to-market losses from instantly eroding solvency (BCBS, 2015).  

From an incentive standpoint, the CVA charge encourages institutions to hedge spread risk using eligible credit default swaps (CDS) or to reduce exposures through netting and collateralization. It also internalizes systemic externalities: when a bank prices trades too tightly without hedging CVA risk, it effectively shifts that risk to creditors and taxpayers. Embedding CVA into regulatory capital ensures that such risks are borne by those who generate them (BCBS & IOSCO, 2015).  

In short, the rationale for the CVA capital charge is threefold:  
1. to close the pre-crisis gap between economic risk and capital coverage;  
2. to dampen procyclical amplifiers of spread-driven losses; and  
3. to improve market discipline by aligning private incentives with systemic stability.  

The following section examines how Basel II first introduced the concept of counterparty credit risk and why its narrow default-only focus necessitated a fundamental redesign under Basel III.
